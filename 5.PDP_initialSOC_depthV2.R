...
# -----------------------------
range_vals <- quantile(FULL_sun2_sub$control_soc_mean_T_ha, probs = c(0.05, 0.99))

pdp_initial_stock <- pdp::partial(
  object = xgb_model,
  pred.var = "control_soc_mean_T_ha",
  train = X_full,
  ice = TRUE,
  center = FALSE,
  pred.grid = data.frame(control_soc_mean_T_ha = seq(range_vals[1], range_vals[2], length.out = 50))
)
...
ggplot(pdp_initial_stock, aes(x = control_soc_mean_T_ha, y = yhat, 
  ...
  geom_point(
    data = FULL_sun2_sub,
    aes(x = control_soc_mean_T_ha, y = seq_rate, color = depth_group),
    inherit.aes = FALSE,
    alpha = 0.3,
  ...
  geom_rug(
    data = FULL_sun2_sub,
    aes(x = control_soc_mean_T_ha, color = depth_group),
    inherit.aes = FALSE,
    sides = "b",
  ...
  group_map(~ {
    tibble(
      ...
      AIC = map_dbl(2:10, function(kk) {
        m <- gam(yhat ~ s(control_soc_mean_T_ha, bs = "cs", k = kk), data = .x)
        AIC(m)
      }),
...
gam_models <- pdp_initial_stock %>%
  group_by(depth_group) %>%
  group_map(~ gam(yhat ~ s(control_soc_mean_T_ha, bs = "cs", k = 4), data = .x), .keep = TRUE)
...
smooth_deriv <- lapply(seq_along(gam_models), function(i) {
  ...
  d <- derivatives(gm, term = "s(control_soc_mean_T_ha)", interval = "confidence", n = 100)
  d$depth_group <- depth
  d
}) %>% bind_rows()
...
summary_deriv <- smooth_deriv %>%
  ...
  summarize(
    ...
    first_significant_time = min(control_soc_mean_T_ha[slope_signif], na.rm = TRUE),
    inflection_times = paste(round(control_soc_mean_T_ha[sign_change], 1), collapse = ", ")
  )

ggplot(smooth_deriv, aes(x = control_soc_mean_T_ha, y = .derivative, color = depth_group)) +
  geom_line() +
  ...
  geom_point(data = subset(smooth_deriv, sign_change == TRUE),
             aes(x = control_soc_mean_T_ha, y = .derivative), color = "black", size = 3) +
...
# -----------------------------
p <- ggplot(pdp_initial_stock, aes(x = control_soc_mean_T_ha, y = yhat, color = depth_group)) +
...
gam_noint <- gam(yhat ~ s(control_soc_mean_T_ha, bs = "cs", k = 6) + depth_group,
                 data = pdp_initial_stock)
...
# 2. Summarize ICE from PDP
pdp_summary <- lapply(target_soc, function(soc) {
  pdp_initial_stock %>%
    group_by(yhat.id) %>%
    slice_min(abs(control_soc_mean_T_ha - soc), n = 1) %>%
    ungroup() %>%
    group_by(depth_group) %>%
    summarize(
      target_soc = soc,
...
# pdp_summary = ICE summary from PDP
pred_df
pdp_summary
...
pdp_unique <- pdp_initial_stock %>%
  group_by(depth_group, control_soc_mean_T_ha) %>%
...
FULL_sun2_sub <- FULL_sun2_sub %>%
  ...
  mutate(
    yhat_num = approx(
      x = pdp_unique$control_soc_mean_T_ha[pdp_unique$depth_group == depth_group],
      y = pdp_unique$yhat[pdp_unique$depth_group == depth_group],
      xout = control_soc_mean_T_ha,
      rule = 2
    )$y
  ) %>%
  ungroup() %>%
...
summary_table <- FULL_sun2_sub %>%
  ...
  summarise(
    n_points = n(),
    rho = cor(yhat_num, control_soc_mean_T_ha, method = "spearman"),
    p_val = cor.test(yhat_num, control_soc_mean_T_ha, method = "spearman")$p.value,
    trend = case_when(
      rho > 0 ~ "increase",
      rho < 0 ~ "decrease",
      TRUE    ~ "none"
...
gam_models <- pdp_initial_stock %>%
  group_by(depth_group) %>%
  group_map(~ gam(yhat ~ s(control_soc_mean_T_ha, bs = "cs", k = 6), data = .x), .keep = TRUE)
