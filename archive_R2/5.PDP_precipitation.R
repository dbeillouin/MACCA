...
# Check for NAs
FULL_sun2_sub[is.na(FULL_sun2_sub$temp_group), c("MEAN_depth", "temp_group")]

# -----------------------------
# 4. Compute Partial Dependence (ICE)
# -----------------------------
range_vals <- quantile(FULL_sun2_sub$precipitation, probs = c(0.05, 0.99))
...
pdp_initial_stock <- pdp::partial(
  ...
  ice = TRUE,
  center = FALSE,
  pred.grid = data.frame(precipitation = seq(range_vals[1], range_vals[2], length.out = 50))
)
...
pdp_initial_stock$temp_group <- rep(FULL_sun2_sub$temp_group, each = n_vals)

vars_to_add <- c("temp_group", "Grouped_Design", "NEW_treatment_type2",
...
# -----------------------------
ggplot(pdp_initial_stock, aes(x = precipitation, y = yhat, 
                              color = temp_group, fill = temp_group)) +
  ...
  geom_point(
    data = FULL_sun2_sub,
    aes(x = precipitation, y = seq_rate, color = temp_group),
    inherit.aes = FALSE,
    alpha = 0.3,
  ...
  geom_rug(
    data = FULL_sun2_sub,
    aes(x = precipitation, color = temp_group),
    inherit.aes = FALSE,
    sides = "b",
...
calc_r2_group <- function(group_name, data_full, model, X_full) {
  idx <- data_full$temp_group == group_name
  y_obs <- data_full$seq_rate[idx]
  y_pred <- predict(model, newdata = X_full[idx, ])
  cor(y_obs, y_pred)^2
}

groups <- unique(FULL_sun2_sub$temp_group)
  ...
  group_map(~ {
    tibble(
      ...
      AIC = map_dbl(2:10, function(kk) {
        m <- gam(yhat ~ s(precipitation, bs = "cs", k = kk), data = .x)
        AIC(m)
      }),
      temp_group = as.character(.y$temp_group)   # récupération correcte du nom du groupe
    )
  }) %>%
  bind_rows()
...
ggplot(aic_results, aes(x = k, y = AIC, color = temp_group)) +
  geom_line() +
  geom_point() +
  theme_minimal()
...
gam_models <- pdp_initial_stock %>%
  group_by(temp_group) %>%
  group_map(~ gam(yhat ~ s(precipitation, bs = "cs", k = 4), data = .x), .keep = TRUE)

names(gam_models) <- levels(pdp_initial_stock$temp_group)

smooth_deriv <- lapply(seq_along(gam_models), function(i) {
  gm <- gam_models[[i]]
  depth <- names(gam_models)[i]
  d <- derivatives(gm, term = "s(precipitation)", interval = "confidence", n = 100)
  d$temp_group <- depth
  d
}) %>% bind_rows()
...
summary_deriv <- smooth_deriv %>%
  ...
  summarize(
    ...
    first_significant_time = min(precipitation[slope_signif], na.rm = TRUE),
    inflection_times = paste(round(precipitation[sign_change], 1), collapse = ", ")
  )

ggplot(smooth_deriv, aes(x = precipitation, y = .derivative, color = temp_group)) +
  geom_line() +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, fill = temp_group), alpha = 0.2, color = NA) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  geom_point(data = subset(smooth_deriv, sign_change == TRUE),
             aes(x = precipitation, y = .derivative), color = "black", size = 3) +
...
# -----------------------------
p <- ggplot(pdp_initial_stock, aes(x = precipitation, y = yhat, color = temp_group)) +
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs", k = 5), se = TRUE)
smooth_data <- ggplot_build(p)$data[[1]]

group_levels <- levels(pdp_initial_stock$temp_group)
smooth_data$temp_group <- factor(group_levels[smooth_data$group], levels = group_levels)
...
gam_depth <- gam(yhat ~ s(precipitation, by = temp_group, bs = "cs", k = 6) +
                   temp_group,
...
gam_noint <- gam(yhat ~ s(precipitation, bs = "cs", k = 6) + temp_group,
                 data = pdp_initial_stock)
...
# Moyenne marginale des prédictions pour chaque temp_group
emm <- emmeans(gam_depth, ~ temp_group)
...
# Target SOC values
target_soc <- c(20, 28)
depths <- levels(pdp_initial_stock$temp_group)
...
pdp_unique <- pdp_initial_stock %>%
  group_by(temp_group, precipitation) %>%
...
FULL_sun2_sub <- FULL_sun2_sub %>%
  ...
  mutate(
    yhat_num = approx(
      x = pdp_unique$precipitation[pdp_unique$temp_group == temp_group],
      y = pdp_unique$yhat[pdp_unique$temp_group == temp_group],
      xout = precipitation,
      rule = 2
    )$y
  ) %>%
...
summary_table <- FULL_sun2_sub %>%
  ...
  summarise(
    n_points = n(),
    rho = cor(yhat_num, precipitation, method = "spearman"),
    p_val = cor.test(yhat_num, precipitation, method = "spearman")$p.value,
    trend = case_when(
      rho > 0 ~ "increase",
      rho < 0 ~ "decrease",
      TRUE    ~ "none"
...
# 7. PDP plot per depth
# -----------------------------
ggplot(pdp_initial_stock, aes(x = precipitation, y = exp(yhat), 
                              color = temp_group, fill = temp_group)) +
  ...
  geom_point(
    data = FULL_sun2_sub,
    aes(x = precipitation, y = exp(yi), color = temp_group),
    inherit.aes = FALSE,
    alpha = 0.3,
    size = 1
  ) +
...
r2_table_RR <- sapply(groups, function(g) {
  idx <- FULL_sun2_sub$temp_group == g
...
gam_models <- pdp_initial_stock %>%
  group_by(temp_group) %>%
  group_map(~ gam(yhat ~ s(precipitation, bs = "cs", k = 6), data = .x), .keep = TRUE)
